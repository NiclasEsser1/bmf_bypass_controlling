...
class RoutingTable(object):
    ...
    def __init__(self, destinations, nbeam, nchunk, nchunk_offset, center_freq_band):
        ...
        # Added by Niclas Esser for BMF bypassing
        elif self.nbeam == 1:
            self.config = BYPASSBMFCONFIG
        else:
        ...
        if self.config == BYPASSBMFCONFIG:
            self.nchunk_expect = self.nchunk
        else:
        ...
        # Added by Niclas for bypassing BMF
        if self.config == BYPASSBMFCONFIG and self.nchunk > 18:
            raise RoutingTableError("In this mode the number of chunks is not allowed to exceed 18. Since a NUMA node can only process one chunk of 36 beams = elements")
    ...
    def generate_table(self):
        ...
        # Added by Niclas for bypassing BMF
        if(self.config == BYPASSBMFCONFIG):
            nbeam_actual = self.config["nbeam_per_nic"]
            nchunk_nic = self.config["nchunk_per_port"]
            nic_idx = 0
            for sb in range(self.first_chunk, self.nchunk + self.first_chunk):
                ...
                for beam in range(nbeam_actual):
                    ...
                    if beam % self.config["nbeams_per_port"] == 0 and beam != 0:
                        port += 1
