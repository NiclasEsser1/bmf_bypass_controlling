...
#             port_dead.append(port_idx)
#
# for val in port_dead:
#     print("removing " +config.numa_list[val].name)
#     config.numa_list.pop(val)
...
for node in config.numa_list:
    ...
    launch_cmd = "python " + script_dir + config.data["launch_script"]["name"] + " -n " +str(node.dockername) + " -c " + cmdline_file + str(node.node_name) + ".txt " + "-d " + config.data["dockerimage"]
